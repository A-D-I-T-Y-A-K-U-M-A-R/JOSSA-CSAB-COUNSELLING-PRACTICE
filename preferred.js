...
/* =========================
   🔥 ADD_TABLE (BULK ADD LIKE ADD BUTTON)
========================= */
document.getElementById("addTableBtn").onclick = function () {

    let previewTable = document.querySelector("#previewTable tbody");
    if (!previewTable) {
        alert("Preview table not found ❌");
        return;
    }

    let rows = previewTable.querySelectorAll("tr");
    ...
    rows.forEach(row => {
        ...
        // 🔥 EXACT SAME SOURCE AS ADD BUTTON SYSTEM
        let inst = row.children[3]?.textContent.trim();
        let branch = row.children[4]?.textContent.trim();

        if (!inst || !branch) return;

        // 🔒 DUPLICATE PROTECTION (same as add button)
    ...
    // ✅ USER FEEDBACK
    alert("Full table added successfully ✅");
